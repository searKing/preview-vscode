...
export class TextEditorHelper {
    ...
    public static indexOf(doc: vscode.TextDocument, startPos: number, marks: string[]): TextEditorHelperReturnType {
        if (!doc) {
    ...
    // 获取指定位置开始后的第一个任意mark的位置
    public static lastIndexOf(doc: vscode.TextDocument, startPos: number, marks: string[]): TextEditorHelperReturnType {
        if (!doc) {
            return TextEditorHelper.NoneTextUtilReturnTypeValue;
        }
        // 获取当前页面文本
    ...
    public static regexIndexOf(doc: vscode.TextDocument, startPos: number, regex: RegExp): TextEditorHelperReturnType {
        if (!doc) {
            return TextEditorHelper.NoneTextUtilReturnTypeValue;
        }
    ...
    public static regexLastIndexOf(doc: vscode.TextDocument, startPos: number, regex: RegExp): TextEditorHelperReturnType {
        if (!doc) {
            return TextEditorHelper.NoneTextUtilReturnTypeValue;
        }
        // 获取当前页面文本
     ...
     */
    public static versionCompare(curVer: string, promoteVer: string): number {
        curVer = curVer || "0.0.0";
